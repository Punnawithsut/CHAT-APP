...
const ChatContainer = () => {
  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
          ...
          <div key={index} className="">
            <div className={`flex items-end gap-2 justify-end 
              ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}>
                {msg.image? (
                  <img src={msg.image} alt='' className='mac-w-[230px] border
      ...
      <div className="absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3">
        <div className="flex-1 flex items-center bg-gray-100/12 px-3 rounded-full">
          <input onChange={(e)=>setInput(e.target.value)} value={input} onKeyDown={(e)=>e.key === "Enter" ? handleSentMessage(e) : null}
    ...
    bg-white/10 max-md:hidden">
      <img onClick={handleSendImage} src={assets.logo_icon} alt='' className='max-w-16'/>
